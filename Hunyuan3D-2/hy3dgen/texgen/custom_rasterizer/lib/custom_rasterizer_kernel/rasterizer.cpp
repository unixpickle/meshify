...
#else
    return rasterize_image_cpu(V.cpu(), F.cpu(), D.defined() ? D.cpu() : D, width, height, occlusion_truncation, use_depth_prior);
#endif
}
...
std::vector<std::vector<torch::Tensor>> build_hierarchy(
    ...
    int resolution)
{
    throw std::runtime_error("build_hierarchy is unavailable in the CPU-only custom_rasterizer build");
}
...
std::vector<std::vector<torch::Tensor>> build_hierarchy_with_feat(
    ...
    int resolution)
{
    throw std::runtime_error("build_hierarchy_with_feat is unavailable in the CPU-only custom_rasterizer build");
}
