...
def rasterize(pos, tri, resolution, clamp_depth=torch.zeros(0), use_depth_prior=0):
    ...
    if findices.device != pos.device:
        findices = findices.to(pos.device)
    if barycentric.device != pos.device:
        barycentric = barycentric.to(pos.device)
    return findices, barycentric


def interpolate(col, findices, barycentric, tri):
    if findices.device != col.device:
        findices = findices.to(col.device)
    if barycentric.device != col.device:
        barycentric = barycentric.to(col.device)
    if tri.device != col.device:
        tri = tri.to(col.device)
