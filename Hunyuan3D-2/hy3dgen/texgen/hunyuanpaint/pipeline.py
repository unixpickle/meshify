...
class HunyuanPaintPipeline(StableDiffusionPipeline):
    ...
    def to(self, *args, **kwargs):
        ...
        if hasattr(pipeline, "solver"):
            pipeline.solver = pipeline.solver.to(pipeline.device)
        return pipeline
    ...
    def __call__(
        ...
        **cached_condition,
        ...
        image_vae = [torch.from_numpy(np.asarray(img, dtype=np.float32) / 255.0) for img in image]
