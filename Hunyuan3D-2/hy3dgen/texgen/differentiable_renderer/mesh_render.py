...
class MeshRender():
    ...
    def set_mesh(
        ...
        self.pos_idx = torch.from_numpy(pos_idx).to(torch.int).to(self.device)
        if (vtx_uv is not None) and (uv_idx is not None):
            self.vtx_uv = torch.from_numpy(vtx_uv).float().to(self.device)
            self.uv_idx = torch.from_numpy(uv_idx).to(torch.int).to(self.device)
        else:
            self.vtx_uv = None
            self.uv_idx = None
    ...
    def render(
        ...
        if tex is not None:
            if isinstance(tex, Image.Image):
                tex = torch.from_numpy(np.asarray(tex, dtype=np.float32) / 255.0)
    ...
    def back_project(self, image, elev, azim,
        ...
        if isinstance(image, Image.Image):
            image = torch.from_numpy(np.asarray(image, dtype=np.float32) / 255.0)
        elif isinstance(image, np.ndarray):
