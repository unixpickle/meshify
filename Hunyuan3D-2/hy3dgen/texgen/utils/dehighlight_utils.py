...
class Light_Shadow_Remover():
    ...
    @torch.no_grad()
    def __call__(self, image, progress_callback=None):
        ...
        if progress_callback is not None:
            progress_callback(0.1, "Prepared relight input")

        def on_step_end(_pipeline, step_index, _timestep, callback_kwargs):
            if progress_callback is not None:
                total_steps = max(self.config.delight_steps, 1)
                progress = 0.1 + (0.75 * (step_index + 1) / total_steps)
                progress_callback(progress, f"Relighting step {step_index + 1}/{total_steps}")
        ...
        image = self.pipeline(
            ...
            height=512,
            width=512,
            num_inference_steps=self.config.delight_steps,
        ...
        if progress_callback is not None:
            progress_callback(1.0, "Light cleanup complete")
